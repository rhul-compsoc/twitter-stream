...
const invertHex = (hex: string) => (Number(`0x1${hex}`) ^ 0xFFFFFF).toString(16).substring(1).toUpperCase();
...
const DVDLogo = (props: DVDLogoProps) => {
  ...
  return (
    <svg className="absolute" ref={svgRef} style={{ top: position.y, left: position.x }}>
      <g>
        <g fill={`#${logoColor}`} style={{ filter: `drop-shadow(3px 3px 1px #${invertHex(logoColor)})` }}>
          <path
